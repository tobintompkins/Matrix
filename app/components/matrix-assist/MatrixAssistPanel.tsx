...
export default function MatrixAssistPanel({
  ...
  const contextLabel = serviceCallId
    ? `Assisting with Service Call ${serviceCallId}`
    : machineId
      ? `Assisting with Machine ${machineId}`
      : "No machine or service call selected";
  ...
  const ensureSession = useCallback(async () => {
    ...
    if (!serviceCallId && !machineId) {
      throw new Error(
        "Select a machine or service call before starting Matrix Assist.",
      );
    }
  ...
  }, [sessionId, serviceCallId, machineId, symptom, observations]);
    ...
    >
      <MatrixCard
        ...
        actions={
          serviceCallId ? (
            <Link
              href={`/service-calls/${serviceCallId}`}
